...
def verify_data(**context):
    ...
    cursor.execute("""
        SELECT 
            'PRICES_RAW' as source, 
            COUNT(*) as count, 
            COUNT(DISTINCT RAW:_ingest_symbol) as distinct_symbols
        FROM PRICES_RAW
        WHERE DATE(LOAD_TS) = (SELECT MAX(DATE(LOAD_TS)) FROM PRICES_RAW)
        GROUP BY 1
    """)
...
def check_data_quality(**context):
    """Check data quality rules on usage tables"""
    import snowflake.connector
    import os
    ...
    cursor.execute("""
        SELECT 
            SUM(CASE WHEN RAW:high::FLOAT < RAW:low::FLOAT THEN 1 ELSE 0 END) as ohlc_violations,
            SUM(CASE WHEN RAW:close IS NULL THEN 1 ELSE 0 END) as null_prices
        FROM PRICES_RAW
        WHERE DATE(LOAD_TS) = (SELECT MAX(DATE(LOAD_TS)) FROM PRICES_RAW)
    """)
    ...
    ohlc_violations = row[0] if row and row[0] else 0
    null_prices = row[1] if row and row[1] else 0
    
    print(f"Quality Check Results:")
    print(f"  OHLC Violations: {ohlc_violations}")
    print(f"  Null Prices: {null_prices}")
    
    if ohlc_violations > 0 or null_prices > 0:
        raise AirflowException(f"❌ Data Quality Failed: {ohlc_violations} OHLC violations, {null_prices} missing prices")
...
with DAG(
    ...
    with TaskGroup('daily_pipeline') as daily:
        ...
        # NOTE: egx_dw must be mounted to /opt/airflow/egx_dw in docker-compose.yml
    ...
    # Updated to use egx_producer.py with environment variables
    stream_data = BashOperator(
        task_id='stream_data',
        bash_command="""
        cd /opt/streaming/producer && \
        export KAFKA_TOPIC=egx_market_data && \
        export FETCH_INTERVAL=60 && \
        timeout 4m python egx_producer.py || true
        """,
    )
    ...
    with TaskGroup('maintenance') as maintenance:
        ...
        quality_check_maintenance = PythonOperator(
            task_id='quality_check_maintenance',
            python_callable=check_data_quality, # Assuming check_data_quality is suitable for maintenance
        )
        ...
        archive = SnowflakeOperator(
            ...
            sql="""
            -- Create archive table if not exists (one-time setup)
            CREATE TABLE IF NOT EXISTS EGYPTIAN_STOCKS.ARCHIVE.PRICES_RAW_ARCHIVE 
            LIKE EGYPTIAN_STOCKS.BRONZE.PRICES_RAW;

            -- 1. Copy data to archive (Append only)
            INSERT INTO EGYPTIAN_STOCKS.ARCHIVE.PRICES_RAW_ARCHIVE
            SELECT * FROM EGYPTIAN_STOCKS.BRONZE.PRICES_RAW
            WHERE LOAD_TS < DATEADD(day, -360, CURRENT_DATE());
            
            -- 2. Delete from operational table
            DELETE FROM EGYPTIAN_STOCKS.BRONZE.PRICES_RAW
            WHERE LOAD_TS < DATEADD(day, -360, CURRENT_DATE());
            """,
        )
        ...
        optimize = SnowflakeOperator(
            task_id='optimize',
            snowflake_conn_id='snowflake_default',
            sql='ALTER TABLE EGYPTIAN_STOCKS.BRONZE.PRICES_RAW RECLUSTER;',
        )
